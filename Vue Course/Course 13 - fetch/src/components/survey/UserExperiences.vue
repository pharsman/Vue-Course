<template>
  <section>
    <base-card>
      ...
      <p v-else-if="!isLoading && (!results || results.length === 0)">No stored experiences found. Star adding some
...
export default {
  ...
  methods: {
    loadExperiences() {
      this.isLoading = true
      this.error = null
      fetch('https://vue-http-demo-155fe-default-rtdb.europe-west1.firebasedatabase.app/surveys.json').then(
        (response) => {
          if (response.ok) {
